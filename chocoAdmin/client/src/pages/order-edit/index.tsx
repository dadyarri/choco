...
export const OrderEdit = () => {
    ...
    const {
        ...
        data: order,
        error: orderError,
    } = useQuery<Order, AxiosError>(["order", orderId], () => getOrderById(orderId!), {
        enabled: orderId !== undefined,
    });

    const { data: products } = useQuery("products", fetchProductsList);
    ...
    const { data: statusesData } = useQuery<OrderStatus[], AxiosError>(
        "orderStatuses",
        fetchOrderStatusesList,
    );

    const { data: citiesData } = useQuery<OrderCity[], AxiosError>(
        "orderCities",
        fetchOrderCitiesList,
    );
    ...
    const validationSchema = Yup.object().shape({
        ...
        orderItems: Yup.array().of(
            Yup.object().shape({
                id: Yup.string()
                    .uuid("Неверный формат идентификатора!")
                    .required("Выбрать продукт обязательно!"),
                amount: Yup.number()
                    .required("Количество товара обязательно!")
                    .positive("Количество товара не может быть меньше или равно нулю"),
            }),
        ),
        address: Yup.object().shape({
            city: Yup.string()
                .uuid("Неверный формат идентификатора")
                .required("Выбрать город обязательно"),
            street: Yup.string()
    ...
    return isOrderLoading ? (
        ...
        <div>
            <Heading as={"h1"} mb={4}>
                {order ? "Редактирование" : "Создание"} заказа
            </Heading>
            <Button
                as={Link}
                colorScheme={"purple"}
                ...
                {({ values, errors, touched }) => (
                    <Form>
                        <VStack spacing={4} align={"flex-start"} mb={4}>
                            ...
                            <FormControl
                                isInvalid={!!errors.address?.building && touched.address?.building}
                                isRequired
                            >
