...
const Home: FC = () => {
    ...
    const {
        ...
        data: statsByCity,
    } = useQuery<StatsSalesByCity, AxiosError>("statsByCity", getStatsByCity, {
        enabled: hasAuthData,
    });
    const {
    ...
    } = useQuery<StatsSalesByProduct, AxiosError>("topProducts", getTopProducts, {
        enabled: hasAuthData,
    });
    ...
    } = useQuery<StatsSalesByCategory, AxiosError>("statsByCategory", getStatsByCategory, {
        enabled: hasAuthData,
    });
            ...
            {!hasAuthData ? (
                <div>
                    <Formik
                        ...
                        onSubmit={async (values) => {
                            const isLoggedIn = await loginByPassword(
                                values.username,
                                values.password,
                            );
                ...
                >
                    <ChartContainer header={"Продажи по городам"}>
                        {isStatsByCityLoading ? (
                            <Spinner />
                        ) : !isStatsByCityError ? (
                            <PieChart data={statsByCity!} />
                        ) : null}
                    </ChartContainer>
                    <ChartContainer header={"Сравнение продаж за последние два месяца"}>
                        {isCompareIncomesLoading ? (
                            <Spinner />
                        ) : !isCompareIncomesError ? (
                    ...
                    <ChartContainer header={"Продажи за последние 10 месяцев"}>
                        {isIncomesStatsLoading ? (
                            <Spinner />
                        ) : !isIncomesStatsError ? (
                            <IncomesChart data={incomesStats!} />
                        ) : null}
                    </ChartContainer>
                    <ChartContainer header={"Продажи по категориям"}>
                        {isStatsByCategoryLoading ? (
                            <Spinner />
                        ) : !isStatsByCategoryError ? (
                            <PieChart data={statsByCategory!} />
                        ) : null}
                    </ChartContainer>
                </Box>
            )}
