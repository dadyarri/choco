...
export const ShipmentEdit = () => {
    ...
    if (shipment) {
        for (let i = 0; i < shipment.shipmentItems.length; i++) {
            shipmentItems.push({
                id: shipment.shipmentItems[i].product.id,
                amount: shipment.shipmentItems[i].amount,
            });
        }
    }
    ...
    const { data: statusesData } = useQuery<ShipmentStatus[], AxiosError>(
        "shipmentStatuses",
        fetchShipmentStatusesList,
    );
    ...
    return isShipmentLoading ? (
        ...
        <div>
            <Heading as={"h1"} mb={4}>
                {shipment ? "Редактирование" : "Создание"} поставки
            </Heading>
            <Button
                as={Link}
                colorScheme={"purple"}
